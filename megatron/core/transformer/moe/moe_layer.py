...
class MoELayer_uniform_distribution(BaseMoELayer):
    ...
    def __init__(
        ...
        self.splits_cpu = torch.tensor([seq_length*config.moe_router_topk//config.num_moe_experts]*config.num_moe_experts, dtype=torch.int32, device=device)
        self.choosed_experts_all_token, _ = generate_scatter_index(
                self.splits_cpu, seq_length, config.moe_router_topk, device
            )
        self.ep_rank = parallel_state.get_expert_model_parallel_rank()
...
class MoELayer_flux_uniform_distribution(BaseMoELayer):
    ...
    def __init__(
        ...
        flux_m_max = 1 * seq_length * config.moe_router_topk
        bf16_moe_args = flux.MoeArguments(
        ...
        if not MoELayer_flux_uniform_distribution._initialized:
            ...
            MoELayer_flux_uniform_distribution.flux_rs_op = flux.GemmGroupedV3GatherRS(config.num_moe_experts, flux_m_max, seq_length, 
                                                        config.moe_router_topk, RANK, WORLD_SIZE, tp_world_size, ep_world_size, 1)
            MoELayer_flux_uniform_distribution._initialized = True

            MoELayer_flux_uniform_distribution.fake_hidden_states = torch.rand((seq_length//WORLD_SIZE, config.hidden_size), dtype=torch.bfloat16, device=device)
            ...
            MoELayer_flux_uniform_distribution.splits_gpu = torch.tensor([seq_length*config.moe_router_topk//config.num_moe_experts]*config.num_moe_experts, dtype=torch.int32, device=device)
            MoELayer_flux_uniform_distribution.splits_cpu = torch.tensor([seq_length*config.moe_router_topk//config.num_moe_experts]*config.num_moe_experts, dtype=torch.int32)
            MoELayer_flux_uniform_distribution.choosed_experts_all_token, MoELayer_flux_uniform_distribution.scatter_index = generate_scatter_index(
                    MoELayer_flux_uniform_distribution.splits_cpu, seq_length, config.moe_router_topk, device
                )
            MoELayer_flux_uniform_distribution.scatter_index = MoELayer_flux_uniform_distribution.scatter_index.to(torch.int32)
            MoELayer_flux_uniform_distribution.outputs = [torch.zeros((seq_length * config.moe_router_topk // ep_world_size, self.config.ffn_hidden_size//tp_world_size), dtype=torch.bfloat16, device=device)]
    ...
    def forward(self, hidden_states):
        ...
        probs0, indices0 = self.router(MoELayer_flux_uniform_distribution.fake_hidden_states.reshape(self.seq_length//self.WORLD_SIZE, 1, self.config.hidden_size))
